...
> 当前实现基线（2026-07-12）：IMDb 已停用 `imdbapi.dev`，全部改由 Just One API；豆瓣短评已停用；缓存采用上映时间动态 TTL，奖项与“你知道吗”使用独立策略。本文保留历史评估章节用于决策追踪；当前权威架构见《项目与架构说明》及源码。
...
- 认证采用服务端 HttpOnly Cookie 会话，Token 原文只在 Cookie 中存在，MySQL 仅保存 SHA-256 摘要。
- 密码使用 Node.js 原生 scrypt、16 字节随机盐和 64 字节派生密钥；不保存明文或可逆密码。
- 登录失败记录写入 MySQL，账号标识与 IP 在 15 分钟内连续失败 8 次后返回 429。
- 非 GET 写请求校验 Origin；Cookie 使用 SameSite=Lax，生产环境自动增加 Secure。
- 用户业务数据通过参数化 SQL 写入 MySQL，不使用 localStorage 保存观影状态、收藏、评分、短评或历史。

## MySQL 表

- users：账号、角色、状态、公开资料和可见性。
- user_sessions：会话摘要、到期、撤销、IP 和 User-Agent。
- auth_login_attempts：数据库级登录失败限流。
- user_media_entries：想看、在看、看过、收藏、评分、短评和审核状态。
- user_browse_history / user_search_history：浏览与搜索历史。
- admin_audit_logs：管理员用户与短评审核操作。
- api_cache / provider_sync_logs：继续承担第三方 API 缓存和同步审计。

## 页面与权限

- /auth：登录注册。
- /me：本人个人中心，包含统计、观影状态、评分短评、收藏、历史和设置。
- /users/:username：公开用户主页；私密主页仅本人可见。
- /admin：管理员概览、用户管理、短评审核和数据源日志。
- 管理员通过 npm run admin:promote -- <用户名或邮箱> 显式提升，不创建默认管理员或默认密码。
...
管理后台由原有单一 `admin` 角色扩展为两层模型：`users.role='admin'` 负责后台准入，`admin_groups`、`admin_group_permissions`、`user_admin_groups` 与 `user_permission_overrides` 负责细粒度 RBAC。权限在服务端实时合并，用户级 `allow/deny` 覆盖分组继承结果；所有管理接口按权限点再次鉴权。

内置超级管理员、内容审核员、接口运维员和数据分析员四组，共 15 个权限点。历史管理员在迁移时自动加入超级管理员组，提升脚本也同步分配该组。

### 后台工作区

后台包含运行总览、统计分析、用户管理、分组与权限、内容审核、API Provider、SQL 缓存、日志中心和站点设置。用户管理支持筛选、分页、详情、风险备注、会话核查和批量处置；内容审核支持查询与批量隐藏/恢复。

### API 与缓存运维

`api_provider_configs` 保存 Provider 名称、基础地址、健康路径、启停、超时、额度说明和测试结果，不保存 API Token。内置 Provider 的启停、地址和超时由真实调用层读取。管理员可以执行服务器侧健康测试，结果进入 `provider_sync_logs` 和审计日志。

`api_cache` 后台支持分页查看、按 Provider 或有效期筛选、精确删除、按来源清理、清理过期记录及手动写入 JSON。所有破坏性操作均要求 `cache.manage` 权限并写审计日志。

### 站点设置

`site_settings` 当前控制开放注册、维护模式、站点公告与短评自动公开。注册开关和维护模式由 API 路由实时执行；短评自动公开关闭时，新短评默认进入隐藏状态。公告通过公开设置接口提供给后续主站组件。

### 数据统计

后台汇总用户新增与活跃、观影状态、评分、短评、会话、缓存体积、Provider 成功率和延迟，并提供 14 天活动、30 天注册、热门作品、热门搜索、搜索类型和时段等分析。统计直接查询 MySQL，不使用前端模拟数据。
