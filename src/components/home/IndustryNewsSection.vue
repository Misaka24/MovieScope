  ...
  <section id="影坛动态" class="scroll-mt-16 py-8">
    ...
    <div v-if="items.length" class="grid gap-5 lg:grid-cols-3">
      <article v-for="news in items" :key="news.id" class="group overflow-hidden rounded-lg border border-white/5 bg-surface-container-low">
        ...
        <div class="p-[18px]"><div class="mb-2.5 flex items-center justify-between gap-3 text-xs font-bold"><span class="text-primary">{{ news.category }}</span><span class="text-on-surface-variant">{{ formatDate(news.publishedAt) }}</span></div><h3 class="text-[17px] font-extrabold leading-[26px] text-on-surface group-hover:text-primary">{{ news.title }}</h3><p class="mt-2.5 line-clamp-3 text-[13px] leading-[21px] text-on-surface-variant">{{ news.summary }}</p><a v-if="news.url" :href="news.url" target="_blank" rel="noreferrer" class="mt-4 inline-flex items-center gap-1 text-[13px] font-bold text-primary">阅读全文<span class="material-symbols-outlined text-lg">arrow_forward</span></a></div>
      </article>
    </div>
