...
<template>
  <article class="poster-card group min-w-0 cursor-pointer">
    <div class="relative aspect-[2/3] overflow-hidden rounded-lg border border-white/5 bg-surface-container">
      ...
      <div class="absolute right-1.5 top-1.5"><span class="rating-badge rounded px-1.5 py-0.5 text-[9px] font-black text-primary-container">{{ imdbLabel(item) }}</span></div>
    </div>
    <h3 class="mt-2 truncate text-sm font-bold text-on-surface transition-colors group-hover:text-primary">{{ item.title }}</h3>
    <p class="mt-0.5 truncate text-xs text-on-surface-variant">{{ item.year || '年份暂无' }} · {{ item.genres.length ? item.genres.slice(0, 2).join(' / ') : item.mediaType === 'tv' ? '剧集' : '电影' }}</p>
  </article>
</template>
