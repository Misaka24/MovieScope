...
function doubanLabel(item: MediaItem) {
  return item.ratings.douban == null ? '豆瓣 未收录' : `豆瓣 ${item.ratings.douban.toFixed(1)}`
}
</script>
  ...
  <article class="poster-card group min-w-0 cursor-pointer">
    <div class="relative aspect-[2/3] overflow-hidden rounded-lg border border-white/5 bg-surface-container">
      ...
      <div class="absolute right-2 top-2 flex flex-col items-end gap-1.5">
        <span class="rating-badge rounded px-2 py-1 text-[10px] font-black text-primary-container">{{ imdbLabel(item) }}</span>
        <span class="rating-badge rounded px-2 py-1 text-[10px] font-black text-secondary">{{ doubanLabel(item) }}</span>
      </div>
    </div>
