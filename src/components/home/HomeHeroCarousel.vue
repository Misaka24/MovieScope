...
<template>
  <section class="relative h-[calc(100svh-4rem)] min-h-[620px] max-h-[900px] w-full overflow-hidden bg-surface-container-lowest">
    <template v-if="ready">
      ...
      <div class="absolute bottom-6 right-4 z-20 flex gap-2 md:right-10"><button v-for="(movie, index) in movies" :key="movie.id" type="button" class="h-1.5 rounded-full transition-all duration-300" :class="index === activeIndex ? 'w-10 bg-primary-container' : 'w-5 bg-white/35'" :aria-label="`切换到${movie.title}`" @click="select(index)"></button></div>
    </template>
