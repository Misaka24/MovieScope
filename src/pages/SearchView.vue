...
<style scoped>
@media(max-width:1023px){.filter-panel{max-height:none;overflow:visible}}
</style>
<style scoped>
.filter-panel{max-height:calc(100vh - 82px);overflow:hidden;padding:15px 16px}.filter-title{margin-bottom:8px;gap:7px;font-size:12px}.filter-title .material-symbols-outlined{font-size:17px}.genre-options{max-height:154px;gap:5px;padding-right:6px}.genre-option span{border-radius:5px;padding:5px 3px;font-size:11px}.year-presets{grid-template-columns:repeat(3,minmax(0,1fr));gap:5px}.year-presets button{border-radius:5px;padding:5px 2px;font-size:10px}.year-presets button:last-child{grid-column:auto}
@media(max-height:760px) and (min-width:1024px){.filter-panel{padding:12px 14px}.genre-options{max-height:124px}.filter-panel.space-y-4>:not([hidden])~:not([hidden]){margin-top:12px}}
</style>
