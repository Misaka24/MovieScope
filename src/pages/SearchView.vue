...
<RouterLink v-else :to="{name:'title',params:{type:item.mediaType,id:item.id}}" class="group grid grid-cols-[112px_1fr] gap-4 overflow-hidden rounded-xl border border-white/5 bg-surface-container p-3 transition-all duration-300 hover:-translate-y-0.5 hover:border-white/10 hover:bg-surface-variant hover:shadow-xl sm:grid-cols-[150px_1fr]"><img :src="item.poster" :alt="item.title" class="aspect-[2/3] w-full rounded-lg object-cover transition-transform duration-500 group-hover:scale-105"><div class="min-w-0 py-1"><div class="flex items-start justify-between gap-3"><div><p class="text-[10px] font-bold uppercase tracking-widest text-primary">{{ item.genres.slice(0,2).join(' · ') || (item.mediaType==='tv'?'剧集':'电影') }}</p><h2 class="mt-1 text-lg font-bold group-hover:text-primary sm:text-xl">{{ item.title }} <span class="text-on-surface-variant">({{ item.year || '年份暂无' }})</span></h2></div><button type="button" class="material-symbols-outlined rounded-full p-1 transition-all hover:bg-white/5 hover:text-primary" :class="isBookmarked(item) ? 'text-primary' : 'text-outline'" :style="isBookmarked(item) ? { fontVariationSettings: '&quot;FILL&quot; 1' } : undefined" :aria-label="isBookmarked(item) ? '取消收藏' : '收藏'" @click.prevent.stop="toggleBookmark(item)">bookmark</button></div><p class="mt-3 line-clamp-3 text-sm leading-6 text-on-surface-variant">{{ item.overview }}</p><div class="mt-4 flex flex-wrap gap-3 text-xs"><span v-if="item.imdbRating!=null" class="rounded bg-[#121212] px-2.5 py-1 font-black text-primary-container">IMDb {{ item.imdbRating.toFixed(1) }} · {{ formatCount(item.imdbVoteCount) }}人</span><span v-else-if="item.tmdbRating!=null" class="rounded bg-[#121212] px-2.5 py-1 font-black text-primary-container">TMDB {{ item.tmdbRating.toFixed(1) }}</span><span>{{ formatRuntime(null) }}</span></div></div></RouterLink>
