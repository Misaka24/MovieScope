...
<template>
  <div class="min-h-screen bg-background text-on-surface">
    ...
    <main>
      ...
      <div id="探索" class="mx-auto max-w-[1216px] px-4 py-6 md:px-8 2xl:max-w-[1480px] 2xl:px-10 min-[1800px]:max-w-[1680px] min-[1800px]:px-12">
        ...
        <div v-else class="space-y-10 py-7"><div v-for="index in 4" :key="index"><div class="mb-4 h-6 w-32 animate-pulse rounded bg-white/5"></div><div class="home-skeleton-grid grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6 2xl:grid-cols-7 min-[1800px]:grid-cols-8"><div v-for="card in 8" :key="card" class="aspect-[2/3] animate-pulse rounded-lg bg-white/5"></div></div></div></div>
      </div>
    </main>
    <GlobalFooter />
  </div>
...
@media (min-width: 1024px) and (max-width: 1535px) {
  .home-skeleton-grid > :nth-child(n + 7) { display: none; }
}
@media (min-width: 1536px) and (max-width: 1799px) {
  .home-skeleton-grid > :nth-child(n + 8) { display: none; }
}
</style>
