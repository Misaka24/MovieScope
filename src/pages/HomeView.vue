  ...
  <div class="min-h-screen bg-background text-on-surface">
    ...
    <main>
      ...
      <div id="探索" class="mx-auto max-w-[1216px] px-4 py-6 md:px-8">
        ...
        <div v-else class="space-y-10 py-7"><div v-for="index in 4" :key="index"><div class="mb-4 h-6 w-32 animate-pulse rounded bg-white/5"></div><div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6"><div v-for="card in 6" :key="card" class="aspect-[2/3] animate-pulse rounded-lg bg-white/5"></div></div></div></div>
      </div>
    </main>
    <GlobalFooter />
  </div>
