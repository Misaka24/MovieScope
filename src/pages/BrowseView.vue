...
<template><div class="min-h-screen bg-background text-on-surface"><GlobalHeader/><main class="mx-auto max-w-[1440px] px-4 pb-16 pt-[82px] md:px-6"><PageState :loading="loading" :error="error" @retry="reload"><template v-if="data"><header class="mb-7 border-b border-white/10 pb-6"><p class="text-xs font-bold uppercase tracking-widest text-primary">MovieScope 榜单</p><h1 class="mt-2 text-3xl font-extrabold md:text-4xl">{{data.title}}</h1><p class="mt-3 text-sm text-on-surface-variant">{{data.description}}</p></header><CatalogPosterGrid :items="data.results" :hide-rating="preset==='upcoming'"/><PaginationBar :page="page" :total-pages="data.totalPages" :total-results="data.totalResults" label="部作品" @change="setPage"/></template></PageState></main><GlobalFooter/></div></template>
